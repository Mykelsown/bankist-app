...
// creates a properties for the balance
function currentAccountBalance(currentAccount) {
  storedCurAcc.balance = currentAccount.movements.reduce(
    (acc, mov) => mov + acc,
    0
  );
  return storedCurAcc.balance;
}
...
const fundsTransfer = function (e) {
  ...
  const executionOfTransfer = function (receiver, sendersMov) {
    if (displayBalance(currentAccount.movements) - amount >= 0 && amount >= 0) {
...
// this function works for loan request
const requestLoan = function (e) {
  e.preventDefault();
  const loanAmount = Number(inputLoanAmount.value);
  if (
    loanAmount > 0 &&
    loanAmount <= currentAccountBalance(currentAccount) * 0.1
  ) {
    const curAcc = accounts.find(
      (acc) => acc.movements === currentAccount.movements
    );
    const movementAfterLoan = [...curAcc.movements, loanAmount];
